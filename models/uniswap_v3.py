...
from .exceptions import NotEnoughLiquidityException
import constants
...
class UniswapV3Pricer:
    ...
    def get_spot_price(self, token_in: str, token_out: str, block_identifier: int) -> decimal.Decimal:
        ...
        sqrt_price_ratio_x96, _ = self.get_slot0(block_identifier)
        if sqrt_price_ratio_x96 == 0:
            return decimal.Decimal(0)
        
        if token_in == self.token0:
            numerator = sqrt_price_ratio_x96 * sqrt_price_ratio_x96
            denominator = (1 << 192)
        else:
            numerator = (1 << 192)
            denominator = sqrt_price_ratio_x96 * sqrt_price_ratio_x96

        price = decimal.Decimal(numerator) / decimal.Decimal(denominator)
        price *= decimal.Decimal(10 ** 6 - self.fee) / decimal.Decimal(10 ** 6)

        return price

    def get_slot0(self, block_identifier, use_cache = True) -> typing.Tuple[int, int]:
        if use_cache == False or self.slot0_cache is None:
            bslot0 = self.get_storage_at('0x0', block_identifier)
    ...
    def get_liquidity(self, block_identifier, use_cache = True) -> int:
        if use_cache == False or self.liquidity_cache is None:
            bliquidity = self.get_storage_at('0x4', block_identifier)
    ...
    def exact_token0_to_token1(self, token0_in: int, block_identifier) -> int:
        (_, ret, __) = self.swap(zero_for_one=True, amount_specified=token0_in, sqrt_price_limitX96=None, block_identifier=block_identifier)
        return -ret

    def exact_token1_to_token0(self, token1_in: int, block_identifier) -> int:
        (ret, _, __) = self.swap(zero_for_one=False, amount_specified=token1_in, sqrt_price_limitX96=None, block_identifier=block_identifier)
        return -ret

    def token0_in_for_exact_token1_out(self, token1_amount_out, block_identifier: int) -> int:
    ...
    def token1_in_for_exact_token0_out(self, token0_amount_out, block_identifier: int) -> int:
    ...
    def get_storage_at(self, key: bytes, block_identifier) -> bytes:
        # convert key to bytes if it isn't already
        if isinstance(key, str):
            if key.startswith('0x'):
                key = key[2:]
            
            # pad to a multiple of 2
            if len(key) % 2 != 0:
                key = '0' + key
        ...
        # first, check cache
        if block_identifier not in self._storage_cache:
            self._storage_cache[block_identifier] = {'n_misses': 0}
        
        cache = self._storage_cache[block_identifier]

        if cache.get('load_failed', None) is False:
            # the storage must be in the cache
            if key not in cache:
                ret = b''.rjust(32, b'\x00')
            else:
                ret = cache[key]
        elif key not in cache:
            cache['n_misses'] += 1

            # if we've had too many misses, load all storage
            if cache['n_misses'] > 20 and 'load_failed' not in cache:
                try:
                    cache.update(_dump_all_storage_at(self.w3, self.address, block_identifier + 1))
                    cache['load_failed'] = False
                except:
                    cache['load_failed'] = True

                ret = self.get_storage_at(key, block_identifier)
            else:
                # otherwise, just load the one storage
                cache[key] = self.w3.eth.get_storage_at(self.address, key, block_identifier)
                ret = cache[key]
    ...
    def get_tick_bitmap_word(self, word_idx, block_identifier, use_cache = True) -> int:
        if not use_cache or word_idx not in self.tick_bitmap_cache:
            ...
            result = self.get_storage_at(h, block_identifier)
            # result = self.get_storage_at(h, block_identifier=block_identifier)
    ...
    def observe_block(self, receipts: typing.List[web3.types.LogReceipt]):
        ...
        for log in receipts:
            ...
            if len(log['topics']) > 0 and bytes(log['topics'][0]) == constants.UNISWAP_V3_SWAP_TOPIC():
                swap = constants._univ3().events.Swap().processLog(log)
            ...
            elif len(log['topics']) > 0 and bytes(log['topics'][0]) in [constants.UNISWAP_V3_BURN_TOPIC(), constants.UNISWAP_V3_MINT_TOPIC()]:
                if log['topics'][0].endswith(constants.UNISWAP_V3_BURN_TOPIC()):
                    event = constants._univ3().events.Burn().processLog(log)
                    ...
                    event = constants._univ3().events.Mint().processLog(log)
                    amount = event['args']['amount']
...
def _dump_all_storage_at(w3: web3.Web3, address: str, block_identifier: typing.Union[str, int]) -> typing.Dict[bytes, bytes]:
    block_hash = _get_block_hash(w3, block_identifier)
    ...
    while next_key is not None:
        ...
        if 'result' not in resp:
            raise Exception(f'error fetching storage: {resp}')

        for item in resp['result']['storage'].values():
            k = item['key']
            v = item['value']
            ret[bytes.fromhex(k[2:])] = bytes.fromhex(v[2:])

        next_key = resp['result']['nextKey']

    return ret

@functools.lru_cache(maxsize=10_000)
def _get_block_hash(w3: web3.Web3, block_identifier: typing.Union[str, int]) -> bytes:
    return w3.eth.get_block(block_identifier)['hash']
